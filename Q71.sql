...
Question: Find all the PC makers who have all their models of PC type in the PC table.
*/

SELECT Maker
FROM
(
SELECT DISTINCT Maker, CASE WHEN B.Model IS NULL THEN -1 ELSE 0 END AS Final   
FROM Product A
LEFT JOIN PC B
ON A.Model = B.Model
WHERE A.Type = 'PC'
