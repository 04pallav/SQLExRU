...
Question: Define the average size of the PC hard drive for each maker that also produces printers.
...
INNER JOIN
  (SELECT Maker,
   ...
   WHERE Maker IN
       (SELECT Maker
        FROM Product
        WHERE TYPE = 'PC' INTERSECT
          SELECT Maker
          FROM Product WHERE TYPE = 'Printer')
     AND TYPE = 'PC') B ON A.Model = B.Model
GROUP BY B.Maker
